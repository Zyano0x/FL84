#pragma once

/**
 * Name: FL84
 * Version: 15.1
 */

#ifdef _MSC_VER
	#pragma pack(push, 0x01)
#endif

namespace CG
{
	// --------------------------------------------------
	// # Structs
	// --------------------------------------------------
	/**
	 * Function Ability_VehicleExplosion.Ability_VehicleExplosion_C.ReceiveBeginPlay
	 */
	struct AAbility_VehicleExplosion_C_ReceiveBeginPlay_Params
	{	};

	/**
	 * Function Ability_VehicleExplosion.Ability_VehicleExplosion_C.ReceiveTick
	 */
	struct AAbility_VehicleExplosion_C_ReceiveTick_Params
	{
	public:
		float                                                      DeltaSeconds;                                            // 0x0000(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	};

	/**
	 * Function Ability_VehicleExplosion.Ability_VehicleExplosion_C.ExecuteUbergraph_Ability_VehicleExplosion
	 */
	struct AAbility_VehicleExplosion_C_ExecuteUbergraph_Ability_VehicleExplosion_Params
	{
	public:
		int32_t                                                    EntryPoint;                                              // 0x0000(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
