...
namespace CG
{
	...
	bool UBP_QuickSummon_Forward_C::TryGetSummonHitResult(struct FHitResult* OutHitResult, class AActor* InSummoner, class USolarSummonDetectionConfig* InSummonConfig)
	{
		...
		if (!fn)
			fn = UObject::FindObject<UFunction>("Function BP_QuickSummon_Forward.BP_QuickSummon_Forward_C.TryGetSummonHitResult");
	...
	UClass* UBP_QuickSummon_Forward_C::StaticClass()
	{
		...
		if (!ptr)
			ptr = UObject::FindClass("BlueprintGeneratedClass BP_QuickSummon_Forward.BP_QuickSummon_Forward_C");
		return ptr;
	}
