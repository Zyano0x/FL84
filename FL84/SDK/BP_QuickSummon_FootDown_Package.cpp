...
namespace CG
{
	...
	bool UBP_QuickSummon_FootDown_C::TryGetSummonHitResult(struct FHitResult* OutHitResult, class AActor* InSummoner, class USolarSummonDetectionConfig* InSummonConfig)
	{
		...
		if (!fn)
			fn = UObject::FindObject<UFunction>(xorstr_("Function BP_QuickSummon_FootDown.BP_QuickSummon_FootDown_C.TryGetSummonHitResult"));
	...
	UClass* UBP_QuickSummon_FootDown_C::StaticClass()
	{
		...
		if (!ptr)
			ptr = UObject::FindClass(xorstr_("BlueprintGeneratedClass BP_QuickSummon_FootDown.BP_QuickSummon_FootDown_C"));
		return ptr;
	}
