...
namespace CG
{
	...
	void UAutomationUtilsBlueprintLibrary::STATIC_TakeGameplayAutomationScreenshot(const class FString& ScreenshotName, float MaxGlobalError, float MaxLocalError, const class FString& MapNameOverride)
	{
		...
		if (!fn)
			fn = UObject::FindObject<UFunction>(xorstr_("Function AutomationUtils.AutomationUtilsBlueprintLibrary.TakeGameplayAutomationScreenshot"));
	...
	UClass* UAutomationUtilsBlueprintLibrary::StaticClass()
	{
		...
		if (!ptr)
			ptr = UObject::FindClass(xorstr_("Class AutomationUtils.AutomationUtilsBlueprintLibrary"));
		return ptr;
	}
